...
int tempAi, tempAp;
  ...
  *-------------------------------------------------------------------*/
  ...
  int* C_index = (int*)malloc_cache_aligned(sizeof(int),matC_nnz,(void*)&C_index_ptr);
  int* C_pointer= (int*)malloc_cache_aligned(sizeof(int),mat_m,(void*)&C_pointer_ptr);



  Entry* C_intermediate_array= (Entry*)malloc(sizeof(Entry)*mat_m);
  ...
  for (int y = 0; y < cores_y; y++)
  {
    for (int x = 0; x < cores_x; x++)
    {
      ...
      kern_args[i] = construct_args(valA, valB, C_kernel,C_intermediate_array, idxA, ptrA, idxB, ptrB, C_pointer,C_index,ptrC,idxC, mat_m, mat_k, i,num_cores);
    }
  }
